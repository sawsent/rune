...
def _build_namespace_tree(full_names: List[str]) -> Dict:
    ...
    return _compact_tree(tree)

def _expand_rich_tree(tree: Tree, node: dict, start_idx: int = 1, sorted_item_list: List[str] = [], current_path: str | None = None) -> Dict[int, str]:
    """Recursively expand the namespace tree into a Rich Tree."""
    idx = start_idx
    items = node.get("__items__", [])
    for item in items:
        tree.add(f"[bold cyan][{idx}][/] {item}")
    ...
    for key, value in node.items():
        ...
        _expand_rich_tree(subtree, value, idx, sorted_item_list, full_path)
